...
| **Contents** | Read-only | Query default branch commit SHA and message |
...
- GitHub App private keys (`GITHUB_APP_PRIVATE_KEY_PATH` or `GITHUB_APP_PRIVATE_KEY`) are kept strictly server-side in the Hub API process.
...
- The `packages/github` adapter enforces runtime read-only checks via `assertReadOnly()`, ensuring mutating calls throw immediately.
